 ...
 * @author jinjunliang
 **/
public class CustomMqttServerConnection {
  ...
  private CustomMqttEnpointImpl endpoint;
  ...
  public CustomMqttServerConnection(NetSocketInternal so, MqttServerOptions options) {
    this.so = so;
    ...
    if (msg instanceof io.netty.handler.codec.mqtt.MqttMessage) {
      ...
      if (result.isFailure()) {
        ...
        if (cause instanceof MqttUnacceptableProtocolVersionException) {
          endpoint = new CustomMqttEnpointImpl(so, null, null, null, false, 0, null, 0);
